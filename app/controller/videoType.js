...
exports.findType = async (reqBody) => {
    try {
        ...
        let list = await videoType.find(data);
...
exports.findTheType = async (reqBody) => {
    try {
        ...
        let result = await videoType.find(where);
...
exports.addType = async (reqBody) => {
    let data = reqBody.data || {};
    try {
        let list = await videoType.find({typeKey: data.typeKey});
        let dataReq = {
        ...
        if(list && list.length > 0) {
            ...
            let result = await videoType.create(dataReq);
...
exports.updateType = async (reqBody) => {
    let data = reqBody.data || {};
    try {
        let list = await videoType.find({_id: data._id});
        let dataReq = {
            typeKey: data.typeKey,
            typeValue: data.typeValue,
            imgGroup: data.imgGroup,
        ...
        let respon = {}
        if(list && list.length > 0) {
            let result = await videoType.update({_id: data._id}, dataReq);
            respon = resdata('0000', 'create success', result);
        } else{
...
exports.removeType = async (reqBody) => {
    ...
    try {
        let list = await videoType.delete({id: data.id});
        return resdata('0000', 'success', list);
    } catch (err) {
        return errdata(err);
    }
