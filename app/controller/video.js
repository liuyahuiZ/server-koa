...
import {videoType} from '../modal/videoType'
...
exports.articleList = async (reqBody) => {
    ...
    try {
        let allList = await video.find(dataArr);
        ...
        let list = await video.find(dataArr, skip, '-content');
...
exports.typeArticleList = async (reqBody, next) => {
    try {
        ...
        let articleList = await video.find(dataArr, skip, '-content');
        //   newList[i] = list[i];
...
exports.articleDetail = async (reqBody) => {
    ...
    try {
        let list = await video.find({_id: reqBody.id});
        ...
        let upResult = await video.update({_id: reqBody.id}, upData);
        return resdata('0000', 'success', {video: list[0], comment: commentlist});
    } catch (err) {
        return errdata(err);
    }
}
...
exports.getArticleDetail = async (reqBody) => {
    // console.log(reqBody.id);
    try {
        let list = await video.find({_id: reqBody._id});
        if(list && list.length > 0) {
            return resdata('0000', 'success', list[0]);
        }else {
            respon = resdata('0013', 'the video is not fond');
        }
...
exports.createArticle = async (reqBody) => {
    ...
    try {
        let newUser = await video.create(dataArr);
...
exports.updateArticle = async (reqBody) => {
    ...
    try {
        ...
        let respon = {};
        if(list && list.length > 0) {
            let updateUser = await video.update({_id: reqBody.id}, dataArr);
...
exports.mkCommit = async (reqBody) => {
    try {
        ...
        let updateUser = await video.update({_id: reqBody.id}, {
...
exports.removeArticle = async (reqBody) => {
    ...
    try {
        ...
        let respon = {};
        if(list && list.length > 0) {
            let list = await video.delete({id: reqBody._id});
