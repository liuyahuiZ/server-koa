...
function getAddrText(text) {
    ...
    return textArr.map((t, i) => {
        return {
            input: getFontImg('/assest/fonts/hei.ttf', t, 33),
            left: 335,
            top: 535 + 48 * i
        };
    });
...
async function getAvatar(img) {
  console.log('img', img, process.cwd(), path.resolve(process.cwd(), img))
  try{
    return await sharp(process.cwd() + img)
        .resize({
            width: 270,
            height: 330
        })
...
function composite(config) {
    return new Promise(async (resolve, reject) => {
        ...
        sharp(process.cwd() + '/assest/images/empty.png')
            .composite(
                [
                    {
                        // 签发机关
                        input: getFontImg(
                            '/assest/fonts/hei.ttf',
                            config.org,
                            33
                        ),
                        left: 525,
                    ...
                    },
                    {
                        // 有效期限
                        input: getFontImg(
                            '/assest/fonts/fzzdxjw-gb1-0.ttf',
                            config.validTerm,
                            33
                        ),
                        left: 525,
